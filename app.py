...
try:
    ...
    pass # Ignore on Windows where native sqlite3 is usually fine
...
<style>
    ...
    .metric-label { font-size: 11px; text-transform: uppercase; letter-spacing: 1.5px; color: #7a8ba8; font-weight: 600; }
    ...
    .chat-ai {
        ...
        padding: 16px 20px;
        margin: 8px 0; margin-right: 10%;
        color: #d0d8e8;
    }

    /* ── Tool Badges ── */
    .tool-badge, .time-badge {
        display: inline-block;
        border-radius: 20px; font-size: 11px; font-weight: 600; padding: 4px 12px;
        margin-top: 8px; margin-right: 6px;
    }
    .tool-badge { background: rgba(0, 255, 180, 0.1); border: 1px solid rgba(0, 255, 180, 0.3); color: #00ffb4; }
    .time-badge { background: rgba(0, 170, 255, 0.1); border: 1px solid rgba(0, 170, 255, 0.25); color: #0af; }

    /* ── Source Box ── */
    .source-box {
        background: rgba(20, 30, 60, 0.5); border-left: 3px solid #00ffb4;
        border-radius: 0 10px 10px 0; padding: 12px 16px; margin: 8px 0;
        font-size: 13px; color: #9aa8c0;
    }
    ...
    .hero-sub { font-size: 14px; color: #6a7b96; margin-bottom: 24px; }
    .sidebar-logo {
        font-size: 22px; font-weight: 800; text-align: center; padding: 12px 0; margin-bottom: 8px;
        background: linear-gradient(135deg, #00ffb4, #0af);
        -webkit-background-clip: text; -webkit-text-fill-color: transparent;
    }

    /* ── SAFELY STYLE UPLOADER CONTAINER (NO BUTTON HACKS) ── */
    ...
    /* Restyle ONLY the uploaded file box */
    [data-testid="stUploadedFile"] {
        background: rgba(13, 19, 51, 0.6) !important;
        border: 1px solid rgba(0, 255, 180, 0.2) !important;
        border-radius: 8px !important;
        color: #e0e6f0 !important;
    }
    
    /* ── Style Uploader Button Text & Icon ── */
    ...
    section[data-testid="stSidebar"] .stFileUploader button svg {
    ...
    /* Keep the small remove (X) button red inside the uploaded file card */
    ...
    .stButton button:hover { border-color: rgba(0, 255, 180, 0.5) !important; color: #00ffb4 !important; }
    .stExpander {
    ...
    if uploaded_file:
        if st.button("Ingest Document", use_container_width=True):
            with st.spinner(""):
                ...
                result = ingestor.ingest_uploaded_pdf(file_bytes, uploaded_file.name, "auto")

                if result["success"]:
                    st.info(f"🤖 Auto-detected domain: **{result['domain'].capitalize()}**")
